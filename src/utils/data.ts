// WaveScan — data.ts (data.js TypeScript 변환)

import type { ScanData, Stats, NotificationItem } from '../types/index';

const SAMPLE_SCANS: ScanData[] = [
  {
    id: 'scan_1',
    ...
    aiComment:
      '해당 용기는 폴리프로필렌(PP 5) 재질로 제작되어 전자레인지 데우기(120°C 이하)에 안전합니다. 뚜껑을 살짝 열거나 증기 배출구를 연 상태에서 사용을 권장합니다.',
  },
  {
    id: 'scan_2',
    ...
    aiComment:
      '금속 테두리나 무늬가 없는 내열 도자기 그릇입니다. 전자레인지 조리 및 데우기에 매우 적합합니다.',
  },
  {
    id: 'scan_3',
    ...
    aiComment:
      '⚠️ 경고: 멜라민 수지는 열에 약하며 가열 시 유해물질이 음식물에 용출될 수 있습니다. 전자레인지 조리를 절대 금지합니다.',
  },
  {
    id: 'scan_4',
    ...
    aiComment:
      '🚫 숟가락, 스테인리스 용기 등 금속 제품은 마이크로파를 반사하여 불꽃을 일으킵니다. 전자레인지에 절대 넣지 마세요.',
  },
];
...
const SAMPLE_NOTIFS: NotificationItem[] = [
  ...
  },
  {
    ...
    type: 'guide',
    title: '안전 가이드 업데이트',
    desc: '알루미늄 호일 사용 안전 수칙이 추가되었습니다.',
    time: '3일 전',
    read: true,
  },
];
...
const notifyDataChange = () => {
  if (typeof window !== 'undefined') {
    window.dispatchEvent(new Event('wavescan_data_change'));
  }
};
...
export const WaveData = {
  getScans(): ScanData[] {
    ...
    return JSON.parse(data) as ScanData[];
  },

  saveScan(newScan: ScanData): ScanData[] {
  ...
  getScanById(id: string): ScanData | undefined {
  ...
  getNotifs(): NotificationItem[] {
    const data = localStorage.getItem(KEYS.NOTIFS);
    if (!data) {
      localStorage.setItem(KEYS.NOTIFS, JSON.stringify(SAMPLE_NOTIFS));
  ...
  hasUnreadNotif(): boolean {
    return this.getNotifs().some((n) => !n.read);
  },
};
